...
public class Job implements Runnable {
	...
	Job(String jId, Boolean inQueue) {
		jobId = jId;
		queued = inQueue;
	...
	public void run() {
		...
		while(running) try {
			if (!queued) {
				...
				setStatus(Status.QUEUED_ON_WORKER);
			}
	...
	private void prepareJob() {
		setStatus(Status.PREPARING_ON_WORKER);
	...
	private void executeJob() {
		...
		setStatus(Status.STARTING_ON_WORKER);
	...
	synchronized void stopJob(Boolean deleteFiles) {
		if ((status != Status.STOPPED) && (status != Status.FINISHED) && (status != Status.ERROR)) {
			setStatus(Status.STOPPING_ON_WORKER);
			if (jobProcess != null) {
				jobProcess.destroy();
			}
		...
		if (deleteFiles) {
			if (jobDirectory.exists()) {
				setStatus(Status.DELETING_ON_WORKER);
...
//================================================================================
	...
	interface StatusChangeListener {
		void onJobStatusChanged(Job job, Status currentStatus) ;
	}
